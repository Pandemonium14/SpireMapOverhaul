package spireMapOverhaul.zones.glassblower.patches;
...
public class MakeDamageMethodUseEffectsQueuePatch {
    ...
    public static class PatchExprEditor extends ExprEditor {
        ...
        public void edit(FieldAccess fieldAccess) throws CannotCompileException {
            if (fieldAccess.getClassName().equals(AbstractDungeon.class.getName()) && fieldAccess.getFieldName().equals("topLevelEffects")) {
                fieldAccess.replace(String.format("{ if (%1$s.active) { $_ = %2$s.topLevelEffectsQueue; } else { $_ = $proceed($$); } }", MakeDamageMethodUseEffectsQueuePatch.class.getName(), AbstractDungeon.class.getName()));
            }
        }
